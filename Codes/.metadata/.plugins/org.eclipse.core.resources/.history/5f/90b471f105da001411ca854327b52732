...
public class SSHConfig {
	...
	public String loadConfigToPlainText(Server sv) {
		...
		try {
			...
			Channel channel = ss.openChannel("exec");
			((ChannelExec) channel)
					.setCommand(" sudo cat /etc/ssh/sshd_config");
			((ChannelExec) channel).setErrStream(System.err);
			BufferedReader br = new BufferedReader(new InputStreamReader(
					System.in));
			InputStream in = channel.getInputStream();
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				if (line.indexOf("Port") != -1
					...
					hm_ssh.put(line.substring(0, line.indexOf(" ")),
			...
			str.close();
			SSH ssh1 = new SSH(hm_ssh.get("Port"), hm_ssh.get("ListenAddress"),
					hm_ssh.get("LoginGraceTime"),
					hm_ssh.get("PermitRootLogin"),
					hm_ssh.get("PasswordAuthentication"),
					hm_ssh.get("PermitEmptyPasswords"),
					hm_ssh.get("AllowUsers"));
			return ssh1;
	...
	public static void main(String[] args) throws IOException {
		...
		SSH ssh2 = ssh_c.convertTextToObject(sv);
		System.out.println("----------------------------");
