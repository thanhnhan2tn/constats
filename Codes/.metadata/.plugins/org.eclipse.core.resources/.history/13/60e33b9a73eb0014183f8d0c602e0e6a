...
public class DHCPConfig {
	...
	// + "dpkg --get-selections | grep 'isc-dhcp-server' | awk '{print $2}'";
	// String kq = uploadToServer(sv, command);
	// String command2 = "echo " + sv.getServerPassword() + " |sudo -S " +
	// " service isc-dhcp-server status";
	// if (kq.startsWith("deinstall") || sendCommandToServer(sv, command2) ==
	// false) {
	// return false;
	// } else {
	// return true;
		...
		while ((line2 = br2.readLine()) != null) {
			if (line2.indexOf("domain-name") != -1 || line2.indexOf("domain-name-servers") != -1
				...
				hm1.put(line2.substring(0, line2.indexOf(" ")).trim(),
						line2.substring(line2.indexOf(" ") + 1).replace(";", "").replace("\"", "").trim());
			}
