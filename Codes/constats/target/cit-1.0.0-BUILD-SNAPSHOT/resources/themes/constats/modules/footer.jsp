<p>Copyright ... 2015</p>
<!-- Script Jquery -->
<script type="text/javascript" src="<c:url value='/resources/themes/constats/js/jquery-2.1.1.min.js'/>"></script>
<!-- Script Jquery-UI -->
<script type="text/javascript" src="<c:url value='/resources/themes/constats/js/jquery-ui.min.js'/>"></script>
<!-- Bootstrap -->
<script type="text/javascript" src="<c:url value='/resources/themes/constats/plugins/bootstrap/js/bootstrap.min.js'/>"></script>
<!-- App Script -->
<script type="text/javascript" src="<c:url value='/resources/themes/constats/js/script.js'/>"></script>
<!-- ContextMenu -->
<script type="text/javascript" src="<c:url value='/resources/themes/constats/plugins/contextmenu/context.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/themes/constats/js/context-server.js'/>"></script>