...
public class ApacheConfig {
	...
	public String loadListenPortToPlainText(Server sv) {
		String kq = sendCommandToServer(sv, "cat /etc/apache2/ports.conf");
		return ChuanHoaChuoi(kq);
	}

	// load config VitualHost to text
	public String loadConfigVirtualHostToPlainText(Server sv) {
		String kq = sendCommandToServer(sv, "cat /etc/apache2/sites-available/000-default.conf");
	...
	public List<VirtualHost> convertTextToVirtualHostObject(Server sv) {
		...
		try {
			while ((line = br.readLine()) != null) {
				if (line.startsWith("VirtualHost") || line.indexOf("ServerName") != -1
						|| line.indexOf("ServerAdmin") != -1 || line.indexOf("DocumentRoot") != -1
						|| line.indexOf("Alias") != -1 || line.indexOf("ErrorLog") != -1
				...
				if (line.endsWith("VirtualHost")) {
					VirtualHost vitualhost = new VirtualHost(hm1.get("VirtualHost"), hm1.get("ServerName"),
							hm1.get("ServerAdmin"), hm1.get("DocumentRoot"), hm1.get("Alias"), hm1.get("ErrorLog"),
							hm1.get("CustomLog"));
	...
	public List<Directory> convertTextToDirectoryObject(Server sv) {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				if (line.startsWith("Directory") || line.indexOf("Options") != -1
						|| line.indexOf("AllowOverride") != -1 || line.indexOf("Require") != -1
						|| line.indexOf("Order") != -1 || line.indexOf("Deny") != -1 || line.indexOf("Allow") != -1) {
					hm1.put(line.substring(0, line.indexOf(" ")).trim(), line.substring(line.indexOf(" ") + 1).trim());
				}
				if (line.endsWith("Directory")) {
					Directory direc = new Directory(hm1.get("Directory"), hm1.get("Options"), hm1.get("AllowOverride"),
							hm1.get("Require"), hm1.get("Order"), hm1.get("Deny"), hm1.get("Allow"));
	...
	public ConfigChung convertTextToObjectConfigChung(Server sv) {
		...
		try {
			HashMap<String, String> hm1 = new HashMap<String, String>();
			StringReader str2 = new StringReader(loadListenPortToPlainText(sv));
			BufferedReader br2 = new BufferedReader(str2);
			String line2 = "";
			while ((line2 = br2.readLine()) != null) {
				line2 = line2.trim();
				if (line2.indexOf("Listen") != -1) {

					hm1.put(line2.substring(0, line2.indexOf(" ")).trim(), line2.substring(line2.indexOf(" ") + 1)
							.trim());
			...
			while ((line = br.readLine()) != null) {
				line = line.trim();
				if (line.indexOf("Timeout") != -1 || line.indexOf("KeepAlive") != -1
						|| line.indexOf("MaxKeepAliveRequests") != -1 || line.indexOf("KeepAliveTimeout") != -1
						|| line.indexOf("ErrorLog") != -1 || line.indexOf("LogLevel") != -1) {
					hm1.put(line.substring(0, line.indexOf(" ")).trim(), line.substring(line.indexOf(" ") + 1).trim());
				}

			}
			str.close();
			ConfigChung cf_chung = new ConfigChung(hm1.get("Listen"), hm1.get("Timeout"), hm1.get("KeepAlive"),
					hm1.get("MaxKeepAliveRequests"), hm1.get("KeepAliveTimeout"), hm1.get("ErrorLog"),
					hm1.get("LogLevel"));
			return cf_chung;
	...
	// Directory
	public Boolean uploadConfigToApacheConfig(Server sv, ConfigChung cfchung, List<Directory> list_direc) {
		String config_tong = "";
		String config = "";
		String config_direc = "";
		System.out.println("------------------Uploading ConfigChung && Directory--------------- ");
		// in ra config chung
		// if (cfchung.getPort() != null && !cfchung.getPort().equals("")) {
		// config = config + "Listen " + cfchung.getPort() + "'\n'";
		// }
		if (cfchung.getTime_out() != null && !cfchung.getTime_out().equals("")) {

			config = config + "Timeout " + cfchung.getTime_out() + "'\n'";
		}
		if (cfchung.getPathErrorLog() != null && !cfchung.getPathErrorLog().equals("")) {
		...
		if (cfchung.getKeepAlive() != null && !cfchung.getKeepAlive().equals("")) {
			config = config + "KeepAlive " + cfchung.getKeepAlive() + "'\n'";
		}
		if (cfchung.getKeepAliveTimeout() != null && !cfchung.getKeepAliveTimeout().equals("")) {

			config = config + "KeepAliveTimeout " + cfchung.getKeepAliveTimeout() + "'\n'";
		}
		if (cfchung.getMaxKeepAliveRequest() != null && !cfchung.getMaxKeepAliveRequest().equals("")) {

			config = config + "MaxKeepAliveRequests " + cfchung.getMaxKeepAliveRequest() + "'\n'";
			config = config + "'\n'";
		}
		...
		for (Directory direc : list_direc) {
			...
			if (direc.getAllowOverride() != null && !direc.getAllowOverride().equals("")) {
				config_direc = config_direc + "AllowOverride " + direc.getAllowOverride() + "'\n'";
			}
		...
		String config_include = "AccessFileName .htaccess " + "'\n'" + "PidFile '${APACHE_PID_FILE}'" + "'\n'"
				...
				+ "'\n'";
		config_tong = "sudo echo -e  > /etc/apache2/apache2.conf " + config_include + config + config_direc;
		// System.out.println("Config tong" + config_tong);
		sendCommandToServer(sv, config_tong);
		String configPort = "";
		if (cfchung.getPort() != null && !cfchung.getPort().equals("")) {
			configPort = configPort + "Listen " + cfchung.getPort() + "'\n'";
		}
		String configPortInclude = "'<'IfModule ssl_module'>''\n'Listen 443'\n''<''/'IfModule'>''\n''<'IfModule mod_gnutls.c'>''\n'Listen 443'\n''<''/'IfModule'>''\n'";
		configPort = "sudo echo -e > /etc/apache2/ports.conf " + configPort + configPortInclude;
		System.out.println(configPort);
		sendCommandToServer(sv, configPort);

		return true;
	}

	// Upload config to /etc/apache2/sites-available/000-default.conf
	public Boolean uploadConfigToVirtualHostConfig(Server sv, List<VirtualHost> list_vitul) {
		System.out.println("------------------Uploading Vitualhost--------------- ");
		...
		config_vth = "sudo echo -e  > /etc/apache2/sites-available/000-default.conf " + config_vth;
	...
	public void inApache(Server sv) {
		...
		System.out.println("Listen " + apache_general.getConfigchung().getPort());
		System.out.println("Timeout " + apache_general.getConfigchung().getTime_out());
		System.out.println("ErrorLog " + apache_general.getConfigchung().getPathErrorLog());
		System.out.println("Loglevel " + apache_general.getConfigchung().getLogLevel());
		System.out.println("KeepAlive " + apache_general.getConfigchung().getKeepAlive());
		System.out.println("KeepAliveTimeout " + apache_general.getConfigchung().getKeepAliveTimeout());
		System.out.println("KeepAliveRequest " + apache_general.getConfigchung().getMaxKeepAliveRequest());
	...
	// convertObjectToXML
	public void convertObjectConfigToXML(ConfigChung cfg, List<Directory> list_direct) {
