...
public class ApacheConfig {
	...
	public boolean createDirectory(Server sv, Directory direc) {
		...
		String command = "sudo echo -e >> /etc/apache2/apache2.conf  " + config_direc;
		LoadOrUpConfigFromServer(sv, command);
	...
	public Boolean createVirtualHost(Server sv, VirtualHost vth) {
		...
		String cmd = "sudo echo -e >> /etc/apache2/sites-available/000-default.conf " + config_vth;
	...
	public String loadConfigToPlainText(Server sv) {
		String kq = LoadOrUpConfigFromServer(sv, "cat /etc/apache2/apache2.conf");
		return kq;
	}

	// load config VitualHost to text
	public String loadConfigVirtualHostToPlainText(Server sv) {
		String kq = LoadOrUpConfigFromServer(sv, "cat /etc/apache2/sites-available/000-default.conf");
		return kq;
	}
	...
	public ConfigChung convertTextToObjectConfigChung(Server sv) {
		...
		try {
			...
			str.close();
			ConfigChung cf_chung = new ConfigChung(null, hm1.get("Timeout"), hm1.get("KeepAlive"),
	...
	// Xoa vitual host
	// Upload config to /etc/apache2/sites-available/000-default.conf
	public Boolean XoaVitulHost(Server sv, List<VirtualHost> list_vitul, String serverName) {
		System.out.println("------------------Uploading Vitualhost--------------- ");
	...
	public void inApache(Server sv) {
		...
		System.out.println("PathErrorLog " + apache_general.getConfigchung().getPathErrorLog());
